...
contains
  ...
  subroutine mini_ch_seulex(T_in, P_in, t_end, VMR, network)
    ...
    ! Work around for high prssure regions
    !if (P_cgs > 20e6_dp) then
    !  iwork(2) = 100 ! Default step numbers - 0 = 100000
    !else
      iwork(2) = 0
    !end if
    ...
    do while((t_now < t_end) .or. (ncall < 100))
